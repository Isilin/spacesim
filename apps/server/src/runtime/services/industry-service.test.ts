...
describe("GameEngine — file de recherche (chantier 11)", () => {
  ...
  const grantScience = (engine: GameEngine, amount: number) =>
    engine.devGrant({ science: amount });
  ...
  it("planifie une chaîne et l'enchaîne recherche après recherche", async () => {
    ...
    expect(engine.snapshotForEmpire(empire).game.research?.techId).toBe(
      "metallurgy",
    );
  ...
  it("refuse une chaîne déjà acquise et sait se vider", async () => {
    ...
    expect(engine.industry.queueResearch(empire, "inconnue")).toBe(
    ...
    engine.devFastForward(3600);
    expect(engine.industry.queueResearch(empire, "metallurgy")).toBe(
      "Technologie déjà acquise",
    );
  });
...
describe("GameEngine — conception de vaisseaux (chantier 13)", () => {
  ...
  it("crée un plan valide, rejette un plan qui dépasse les emplacements", async () => {
    ...
    expect(
      engine.industry.createBlueprint(
        a,
        "Mon cargo",
        COLONY_BP.chassisId,
        COLONY_BP.modules,
  ...
  it("construit un plan de domaine colonie : file navale puis livraison", async () => {
    ...
    const plan = snap(engine, a).blueprints.find(
      (p) => p.chassisId === "light_freighter",
    )!;
    expect(engine.industry.buildBlueprint(a, plan.id, colony.id)).toBeNull();
    expect(
      snap(engine, a).colonies[0]!.shipQueue.some((q) => q.shipId === plan.id),
    ).toBe(true);
    advanceTicks(engine, 60);
    expect(
      snap(engine, a).colonies[0]!.ships[plan.id] ?? 0,
  ...
  it("construit un plan de domaine flotte : file de la flotte", async () => {
    ...
    engine.devGrant({ metals: 1000, components: 500 });
    expect(
      engine.fleetService.createFleet(a, colony.id, "1re escadre"),
    ).toBeNull();
    const fleetId = snap(engine, a).fleets[0]!.id;
    const plan = snap(engine, a).blueprints.find(
      (p) => p.chassisId === "scout_frame",
    )!;
    const err = engine.industry.buildBlueprint(a, plan.id, undefined, fleetId);
    expect(err).toBeNull();
    expect(
      snap(engine, a).fleets[0]!.queue.some((q) => q.warshipId === plan.id),
    ).toBe(true);
  });
  ...
  it("refuse de bâtir un plan de flotte au chantier civil (mauvais domaine)", async () => {
    ...
    const a = empireFor(engine, "alice");
    const colony = snap(engine, a).colonies[0]!;
    const fleetPlan = snap(engine, a).blueprints.find(
      (p) => p.chassisId === "scout_frame",
    )!;
    expect(
      engine.industry.buildBlueprint(a, fleetPlan.id, colony.id),
    ).not.toBeNull();
  });
  ...
  it("les plans survivent au rechargement", async () => {
    ...
    expect(e2.snapshotForEmpire(a2).blueprints.map((p) => p.name)).toContain(
      "Persistant",
    );
  });
  ...
  describe("marché de plans en comptoir", () => {
    it("achète un plan de départ : crédits débités, plan ajouté", async () => {
      ...
      expect(after.colonies[0]!.resources.credits).toBeLessThan(
    ...
    it("revend un plan : crédité, plan retiré", async () => {
      ...
      expect(
        engine.industry.sellBlueprint(a, colony.id, comptoir.id, plan.id),
      ).toBeNull();

      const after = snap(engine, a);
      expect(after.blueprints.some((p) => p.id === plan.id)).toBe(false);
      expect(after.colonies[0]!.resources.credits).toBeGreaterThan(
        creditsBefore,
      );
    });
    ...
    it("revend un vaisseau assemblé (classe historique) : décompte le pool, crédite", async () => {
      ...
      expect(
        engine.industry.sellShip(a, colony.id, comptoir.id, "cargo_small", 1),
      ).toBeNull();
    ...
    it("refuse de vendre plus de vaisseaux que disponibles", async () => {
      ...
      const colony = snap(engine, a).colonies[0]!;
      expect(
        engine.industry.sellShip(a, colony.id, comptoir.id, "cargo_small", 99),
      ).not.toBeNull();
    });
  });
});
/** Crée un plan nommé « Persistant » (helper du test de persistance). */
function engine_createNamed(
  engine: GameEngine,
  empire: ReturnType<typeof empireFor>,
): void {
